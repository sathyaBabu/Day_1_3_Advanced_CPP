//
//  main.cpp
//  12_1_IntrepreterWithExpression_Flow    fold
//
...
class Calculator {
public:
    std::unique_ptr<Expression> parse(const std::string& input) {  //  "2 3 4 * +"
...
int evaluateExpressions( Calculator& calculator, const std::string& input, Args&&... args) {
    ...
    std::vector<std::string> expressions{ std::forward<Args>(args)... };  // check on ohase II
    int result = 0;
    int argsCount = 0 ;
